...
describe("when rendered", () => {
    ...
    it("returns transformed value", () => {
      ...
      const { result } = renderHook(
        () => useInputList());
        act(() => result.current.handleInputChange(event));      
      expect(result.current.outputValue).toBe("one thousand nine hundred and ninety-nine");
    });
    it("returns transformed value", () => {
    ...
    it("returns transformed value", () => {
      ...
      const { result } = renderHook(
        () => useInputList());
        act(() => result.current.handleInputChange(event));      
      expect(result.current.outputValue).toBe("seventeen thousand nine hundred and ninety-nine");
    });
    it("returns transformed value", () => {
      ...
      const { result } = renderHook(
        () => useInputList());
        act(() => result.current.handleInputChange(event));      
      expect(result.current.outputValue).toBe("three hundred and forty-two thousand two hundred and fifty-one");
    });
    it("returns transformed value", () => {
      ...
      const { result } = renderHook(
        () => useInputList());
        act(() => result.current.handleInputChange(event));      
      expect(result.current.outputValue).toBe("one million three hundred thousand four hundred and twenty");
    });
  });